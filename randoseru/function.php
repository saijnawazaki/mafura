<?php
function getEchoHeaderImportant($settings)
{
	$content = $settings['content'];
	
	preg_match_all('/(\@echo-header-important+[\s]+(\'|\")+[a-zA-Z0-9\(\)\-\_\@\/\*\!\.\,\s\r\n\|\:]+(\'|\")+\;+$)/m', $content, $matches);
	...
	{
		$exp_con = explode('@echo-header-important',$content_matches);
		$file_inc = $exp_con[1];
	...
	{
		return ['echo_header_important'=>$comment_list,'content'=>$content];	
	}
	else
	{
...
function cssToArray($settings)
{
	...
	return isset($content_list) ? $content_list : array();
}
